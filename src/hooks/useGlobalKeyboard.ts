...
export const useGlobalKeyboard = () => {
  const setIsTerminalOpen = useGlobalStore((s) => s.setIsTerminalOpen)
  const isTerminalOpen    = useGlobalStore((s) => s.isTerminalOpen)
  ...
  useEffect(() => {
    ...
    const handleKey = (e: KeyboardEvent) => {
      ...
      // Don't intercept inside inputs / terminal
      const inInput = tag === 'INPUT' || tag === 'TEXTAREA'

      // ~ key → toggle terminal (not in input)
      if (e.key === '`' && !inInput) {
        e.preventDefault()
      ...
      if (e.key === KONAMI[konamiProgress]) {
        ...
        if (konamiProgress === KONAMI.length) {
          konamiProgress = 0
          triggerKonami(setIsGlitchMode, isGlitchMode)
        }
      } else {
        konamiProgress = 0
      }
...
const triggerKonami = (setIsGlitchMode: (v: boolean) => void, current: boolean) => {
  // Full-screen glitch flash effect
  ...
  style.textContent = `
    @keyframes konami-flash {
      0%   { opacity:1; transform:skewX(-5deg) }
      20%  { opacity:0.5; transform:skewX(5deg) }
      40%  { opacity:1; transform:skewX(-3deg) }
      60%  { opacity:0.3; transform:skewX(3deg) }
      80%  { opacity:0.8; transform:skewX(-1deg) }
  ...
  msg.style.cssText = `
    ...
    text-align:center;pointer-events:none;letter-spacing:0.2em;
    animation:konami-msg 2.5s ease forwards;
  `
