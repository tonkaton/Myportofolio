...
const MODE_CONFIG: Record<CursorMode, {
  ...
  default:   { dotSize: 8,  dotColor: '#00ffff', dotShadow: '0 0 8px #00ffff, 0 0 16px rgba(0,255,255,0.4)',   ringSize: 32, ringBorder: '1px solid rgba(0,255,255,0.6)',  ringShadow: '0 0 6px rgba(0,255,255,0.3)',  trailColor: '0,255,255',   trailMaxAlpha: 0.45 },
  laser:     { dotSize: 4,  dotColor: '#ff0040', dotShadow: '0 0 12px #ff0040, 0 0 24px rgba(255,0,64,0.6)',   ringSize: 24, ringBorder: '1px solid rgba(255,0,64,0.8)',   ringShadow: '0 0 10px rgba(255,0,64,0.5)', trailColor: '255,0,64',    trailMaxAlpha: 0.6  },
  crosshair: { dotSize: 2,  dotColor: '#ffff00', dotShadow: '0 0 6px #ffff00',                                 ringSize: 36, ringBorder: '1px solid rgba(255,255,0,0.7)', ringShadow: '0 0 8px rgba(255,255,0,0.3)', trailColor: '255,255,0',   trailMaxAlpha: 0.35 },
  neon:      { dotSize: 10, dotColor: '#ff00ff', dotShadow: '0 0 14px #ff00ff, 0 0 28px rgba(255,0,255,0.5)', ringSize: 40, ringBorder: '2px solid rgba(255,0,255,0.7)',  ringShadow: '0 0 12px rgba(255,0,255,0.4)',trailColor: '255,0,255',   trailMaxAlpha: 0.5  },
}
...
export const CursorTrail = () => {
  ...
  // Subscribe to store without triggering re-render
  useEffect(() => {
    const unsub = useGlobalStore.subscribe((s) => {
      if (s.cursorMode === modeRef.current) return
  ...
  const applyMode = (mode: CursorMode) => {
    ...
    if (hl) hl.style.display = mode === 'crosshair' ? 'block' : 'none'
    if (vl) vl.style.display = mode === 'crosshair' ? 'block' : 'none'
  }
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      if (dotRef.current)  dotRef.current.style.transform  = `translate(${raw.x - half}px,${raw.y - half}px)`
      if (hLineRef.current) hLineRef.current.style.transform = `translateY(${raw.y}px)`
      if (vLineRef.current) vLineRef.current.style.transform = `translateX(${raw.x}px)`
      trail.push({ x: raw.x, y: raw.y })
      if (trail.length > TRAIL) trail.shift()
    }

    // FIX: onDown tidak lagi parse transform string — simpan ring pos di ref
    ...
    const tick = () => {
      ...
      if (ringRef.current) ringRef.current.style.transform = `translate(${ring.x - half}px,${ring.y - half}px)`
